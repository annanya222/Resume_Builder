...
const Dashboard = () => {
  ...
  const [showUploadResume, setShowUploadResume] = useState(false)  // ✅ fixed casing
  ...
  const uploadResume = async (event) => {
    ...
    try {
      ...
      if (!resumeText?.trim()) {
        toast.error('Could not extract text from PDF. Is it a scanned/image PDF?')
        setIsLoading(false)
        return
      }
      ...
      <div className='max-w-7xl mx-auto px-4 py-8'>
        ...
        <div className='flex gap-4'>
          ...
          <button
            ...
            <p className='text-sm group-hover:text-purple-600 transition-all duration-300'>Upload Existing</p> {/* ✅ fixed typo */}
          </button>
        ...
        <div className="grid grid-cols-2 sm:flex flex-wrap gap-4">
          {allResumes.map((resume, index) => {
            ...
            return (
              <button
                key={resume._id}  // ✅ fixed key to use _id
            ...
            <div onClick={e => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
              ...
              <div>
                <label htmlFor="resume-input" className="block text-sm text-slate-700">
                  Select resume file
                  <div className='flex flex-col items-center justify-center gap-2 border group text-slate-400 border-slate-400 border-dashed rounded-md p-4 py-10 my-4 hover:border-green-500 hover:text-green-700 cursor-pointer transition-colors'> {/* ✅ fixed hover typo */}
                    {resume ? (
              ...
              {/* ✅ Fixed button - removed duplicate text */}
              <button
                disabled={isLoading}
        ...
        {editResumeId && (
          <form onSubmit={editTitle} onClick={() => setEditResumeId('')} className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center'> {/* ✅ fixed setEditResume -> setEditResumeId */}
            <div onClick={e => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
